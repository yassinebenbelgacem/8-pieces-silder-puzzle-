...
from algorithms import astar_solver, bfs_solver
...
class Game:
    ...
    def new(self):
        ...
        self.buttons_list = [
            ...
            Button(500, 240, 200, 50, "A*", WHITE, BLACK),
            Button(500, 310, 200, 50, "BFS", WHITE, BLACK)
        ]
        self.draw_tiles()
    ...
    def update(self):
        ...
        if self.solver_path:
            if self.solver_index < len(self.solver_path):
                self.tiles_grid = [list(self.solver_path[self.solver_index][i:i + GAME_SIZE]) for i in range(0, len(self.solver_path[self.solver_index]), GAME_SIZE)]
                self.solver_index += 1
                self.draw_tiles()
            else:
                self.solver_path = []
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                for button in self.buttons_list:
                    if button.click(mouse_x, mouse_y):
                        ...
                        if button.text == "BFS":  # Handle BFS button click
                            self.solve_with_bfs()

    def solve_with_astar(self):
        self.solver_path = astar_solver(self.tiles_grid, self.tiles_grid_completed)
        self.solver_index = 0

    def solve_with_bfs(self):
        try:
            self.solver_path = bfs_solver(self.tiles_grid, self.tiles_grid_completed)
            self.solver_index = 0
            print("BFS solution found:", self.solver_path)
        except Exception as e:
            print("Error occurred during BFS solving:", e)
